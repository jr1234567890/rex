...
from rex_BodyDetectionFunctions import DetectBody      #The face detector, using HOG, and the Neural net stick
...
myDetectBody = DetectBody(frame, proc_w, proc_h)
...
while(True):  # replace with some kind of test to see if WebcamStream is still active?
    ...
    rects=myDetectBody.update(frame,conf["detection_confidence"])
