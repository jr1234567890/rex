...
from rex_MotionDetectionClass import MotionDetect
...
while(True):  # replace with some kind of test to see if WebcamStream is still active?
    ...
    (motion_flag, motionx, motiony) =myDetector.get_detection_flag()  #get the latest motion detection flag
    clean=myFrameCapture.getFrame()
    myDetector.newFrame(clean)           #feed the latest frame to the motion detector
    #print (motion_flag, motionx, motiony)
    ...
    #if there are no face detections, look for motion flag 
    if len(current_list)==0:        

        #print(motion_flag, motionx, motiony)
        #check for motion detector flag
        if motion_flag==1:
            target_x=motionx
            target_y=motiony
            #print ("target selected by motion detector ", motionx, motiony)


        else: #there is no face, and no motion

            #set the target to the neutral point
            #target_x = proc_w/2
            target_y = proc_h/3  #set it to look up
            tilt_servo=90
            eye_angle=0
            ID_object=999   #set the recognizer ID to default
            #print ("no detections")

            #set up a slow left right scan
            if shake_flag==True:  #this is a continuation of a shaking motion

                head_pos=head_pos + 5*direction
                if head_pos>proc_w/2+200:
                    direction =-1
                if head_pos<proc_w/2-200:
                    direction =1
            else:  #this is the start of the shaking motion
                head_pos=proc_w/2
                direction=1
                shake_flag=True
            target_x= head_pos
            #print ("target selected by head shake", target_x)

    else:  #else process the detection(s)
        shake_flag=False  #set the shake flag, to indicate that shaking has stopped because we've seen something
